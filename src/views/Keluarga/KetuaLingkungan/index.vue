...
export default {
  ...
  async mounted() {
    this.lingkungan = await getOneData(`/lingkungan/${this.$store.state.keluarga.lingkunganId}`)
  ...
  methods: {
    getSuratNotApprovedByKetuaLingkungan() {
      this.cards.map(async (card) => {
        let tempData = await getData(`/${card.endpoint}/lingkungan/${this.$store.state.keluarga.lingkunganId}`)
        
        card.unverifiedCount = tempData.filter(surat => surat.ketua_lingkungan_approval != 1 && surat.deleted_at === null)
